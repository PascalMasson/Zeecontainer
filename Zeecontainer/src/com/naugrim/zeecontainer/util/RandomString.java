...
public class RandomString {
	...
	public static String generateRandomString(int len) {
		...
		for (int i = 0; i < len; i++) {
			sb.append(UpperLowerCaseLetters
					.charAt(r.nextInt(UpperLowerCaseLetters.length())));
		}
		return sb.toString();
	...
	public static String generateRandomZipCode(String template) {
		...
		for (int i = 0; i < template.length(); i++) {
			if (template.charAt(i) == 'n') {
				...
				sb.append(UpperCaseLetters.charAt(
						new Random().nextInt(UpperCaseLetters.length())));
			} else {
				sb.append(" ");
			}
		}
